...
class AdapNet_pp(network_base.Network):
    def __init__(self, num_classes=12, learning_rate=0.001, float_type=tf.float32, weight_decay=0.0005,
        ...
        with tf.variable_scope('v'):
            v = 0.5*tf.ones(tf.shape(self.softmax))
    ...
    def _setup(self, data):   
        ...
        #################################################################
        #CODE WRITTEN BY US, Check this if stuff starts cupping
'''
        self.lam = 5e-6*self.global_step + 0.3
        self.l   = tf.clip_by_value(self.softmax,clip_value_min = -1.0,clip_value_max = self.lam)
        self.v   = tf.add(1,tf.divide(self.l,self.lam))
'''
        #################################################################
        
    def _create_loss(self, label):
        
        lam = 5e-6*self.global_step +  0.3       
        self.loss = tf.reduce_mean(-tf.reduce_sum(tf.multiply(tf.multiply(label*tf.log(self.softmax+1e-10), self.weights),v), axis=[3])) + lam*(tf.subtract(0.5*tf.square(v),v))
        '''
        if self.has_aux_loss:
        ...
        ###############################################
        #CODE WRITTEN BY US,Check this if stuff starts cupping
        self.loss = self.loss + tf.reduce_mean(self.lam*(tf.subtract(tf.square(self.v)/2,self.v)),axis = [3])
        ###############################################
        '''
    def create_optimizer(self):
        self.lr = tf.train.polynomial_decay(self.learning_rate, self.global_step,
                                            self.decay_steps, power=self.power)
        v_vars = [var for var in tf.trainable_variables() if 'v' == var.name]
        w_vars = [var for var in tf.trainable_variables() if 'v' != var.name]
        self.train_op = tf.train.AdamOptimizer(self.lr).minimize(self.loss, global_step=self.global_step,var_list = v_vars)
        self.train_op2 = tf.train.AdamOptimizer(self.lr).minimize(self.loss, global_step=self.global_step,var_list = w_vars)
